...
- **CI deprecation warnings**: the Actions run currently surfaces two non-blocking notices — `actions/checkout@v4`/`actions/setup-node@v4`/`actions/upload-artifact@v4` targeting the soon-deprecated Node 20 runtime, and an `ubuntu-latest` image migration heads-up for October 2026. Neither fails the build; noticed on the final run but left as-is since the time-box had already elapsed by then.
...
- A second independent Opus review was used to sanity-check a *process* decision, not code: after splitting this work into three stacked pull requests (using GitHub's new native stacked-PR feature), it was asked whether that was appropriately scoped for a 2-3 hour take-home. Its verdict was no — a stack's purpose is landing independent units, and this was a mid-session bug fix, not one — so the work was collapsed back into this single PR, keeping the individual commits. Full reasoning for both the split and the collapse is in `DIARY.md`.
